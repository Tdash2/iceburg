...
?>
<?php
// videohub_output_labels.php
// Only allows editing of Videohub output labels
...
$VIDEHub_PORT = 9990;
$SOCKET_TIMEOUT = 2.0; // seconds for connect/read
...
<head>
<meta charset="utf-8">
<title>Videohub Output Labels</title>
...
<script>
let outputLabels = [];
let changedLabels = {}; // portIndex -> new label
...
async function refreshLabels(){
    ...
    try {
        ...
        outputLabels = res.output_labels || [];
        const count = res.output_count || outputLabels.length;

        let html = '<table><thead><tr><th>Output Number</th><th>Label</th></tr></thead><tbody>';
        for(let i=0;i<count;i++){
            const lab = outputLabels[i] || '';
            html += `<tr>
                        <td>${i+1}</td>
...
function markChanged(input){
    ...
    const original = outputLabels[index] || '';
    if(input.value !== original){
        input.style.backgroundColor = 'rgb(153 255 156)'; // red
...
document.getElementById('submitChanges').addEventListener('click', async ()=>{
    ...
    for(const index in changedLabels){
        ...
        const res = await api('set_label',{ type:'output', port:parseInt(index)+1, label: label });
        if(res.ack !== 'ACK'){
